...
Emozzk는 CHZZK 기본 이모티콘 패널을 키보드 단축키와 즐겨찾기 기능으로 더 빠르게 조작할 수 있게 해주는 Chromium 계열 브라우저 확장 프로그램입니다.

CHZZK 이모티콘을 키보드 단축키로 입력할 수 있게 합니다.
...
[**NEW!**] Emozzk 설정 화면에서 세트 전환 단축키를 지정할 수 있습니다.
- 기본값 미지정
- 사용자 편의에 맞추어 `Ctrl + ←`, `Ctrl + →` 등으로 지정 가능
- 라이브 채널에 진입 시 현재 활성화된 세트를 이모티콘 버튼 위치에 표시
- 이모티콘 패널이 닫혀있을 때 세트를 전환하면 활성 세트를 이모티콘 버튼 위치에 표시
...
조합 키(Ctrl, Shift 등) 는 단독으로 사용할 수 없습니다.
...
단축키를 해제할 이모티콘을 선택한 뒤 <span class="doc-icon doc-icon-save" role="img" aria-label="저장"></span> 버튼을 누르면 선택한 단축키가 해제됩니다.
...
<p>
  <img src="./assets/pngs/popup.png" width="332" alt="Emozzk 확장 프로그램 팝업 설정">
</p>
...
`단축키 입력 시 이모티콘 패널 숨기기`를 활성화하면 단축키로 이모티콘을 사용할 때 이모티콘 패널이 표시되지 않게 할 수 있습니다.
...
- Emozzk는 CHZZK 메시지 전송 데이터나 WebSocket 데이터를 직접 수정하지 않습니다.
- 채팅 입력창에 이미 이모티콘이 10개 들어간 경우에는 추가 이모티콘 입력을 시도하지 않습니다.
- 즐겨찾기, 단축키, 최근 이모티콘 설정은 브라우저 로컬 저장소에 저장됩니다.
- Emozzk는 설정 저장을 위해 `storage` 권한을 사용하고, CHZZK 페이지에서 동작하기 위해 `https://chzzk.naver.com/*` 권한을 사용합니다.
